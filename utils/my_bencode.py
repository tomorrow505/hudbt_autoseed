...
def change_tracker(torrent_path):
    back_path = torrent_path+'TTT.torrent'
    os.rename(torrent_path, back_path)
    data = bencode.bread(back_path)
    if not data["announce"].startswith('https'):
        data["announce"] = data["announce"].replace('http', 'https')
    bencode.bwrite(data, torrent_path)
    os.remove(back_path)


def change_tracker_2_mteam(torrent_path):
    back_path = torrent_path + 'TTT.torrent'
    os.rename(torrent_path, back_path)
    data = bencode.bread(back_path)
    data["announce"] = 'http://pt.m-team.cc/announce.php'
    bencode.bwrite(data, torrent_path)
    os.remove(back_path)


# if __name__ == "__main__":
#     # 0a42605a2a3a4aabddfede1901154359d59a0947
#     path = r'E:\HUDBT\呵呵.torrent'
#     print(get_hash(path))
